...
class tv_DL_filter extends content_DL_filter
{
    ...
    protected function parseFilter($filter)
    {
        ...
        if (parent::parseFilter($filter)) {
            ...
            if (!$this->tv_id) {
                $tvid = $this->DocLister->dbQuery("SELECT id FROM " . $this->DocLister->getTable('site_tmplvars') . " WHERE `name` = '" . $this->modx->getDatabase()->escape($this->field) . "'");
                $this->tv_id = (int)$this->modx->getDatabase()->getValue($tvid);
            }
